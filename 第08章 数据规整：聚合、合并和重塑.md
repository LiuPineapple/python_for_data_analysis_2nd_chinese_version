...
1. 如何创建层次化索引
   - 创建Series或者DataFrame时，通过嵌套列表创建层次化索引
   - 通过MultiIndex.from_arrays方法，传入嵌套列表

2. MultiIndex对象有.names属性，类似于Index对象的.name属性
3. 层次化索引可以类似行列索引进行索引和切片，也可以类似重复索引一样使用索引达成切片的功效
...
left,right,how,on,left_on,right_on,left_index,right_index,copy
...
1. 详情见官方文档：https://pandas.pydata.org/docs/reference/api/pandas.concat.html?highlight=concate

2. 需要记住的参数有：objs,axis,join,keys,join_axes,ignore_index
3. 合并时，原有的行/列索引会保留，除非ignore_index，此时axis=轴上的索引会删除重新变成range。keys无论是变成索引还是变成层次化索引都是为了区分开合并的多个数据集。可以用names为层次化索引加名字
4. concat与merge的区别：merge仅用于横向合并，而且必须选择列或者索引作为合并键，类似数据库风格。
