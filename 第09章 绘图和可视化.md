...
jupyter notebook关于matplotlib有几个魔术指令

https://blog.csdn.net/zeng_xiangt/article/details/81629528

https://blog.csdn.net/qq_26822029/article/details/103064856

网上的说法是使用%matplotlib inline之后图片就会直接内嵌画图，不用输入plt.show()命令，而是直接显示图像，但是在实际中就算不加这一魔术命令所画图片也会自动显示不用plt.show()，不知道为什么

ipython关于matplotlib有魔术指令%matplotlib，和jupyter里面的%matplotlib notebook一个作用，不用plt.show()即可展示图片，同时可以进行交互
...
1. 在Matplotlib库中提供了两种风格的API供开发者使用：一种是Pyplot编程接口（state-based）/Matlab风格，一种是面向对象对象的编程接口（object-based）。前者是封装好的较为简洁的，后者是更为灵活，功能更为强大的。https://blog.csdn.net/theonegis/article/details/81230211
2. 了解一下matplotlib的backends，不需要了解太深
...
一些参数需要掌握，facecolor,figsize

```sql
fig = plt.figure(figsize=plt.figaspect(0.618),facecolor='b')
...
1. plt.plot()接受x值与y值，如果只给一个向量的话默认为y值，x值为0到N-1（N为传入向量长度）。对于x和y来说，可以传入很多类型，比如list,np.ndarray等。也可以不传入x,y而是在其位置上放index/key值，在后面的data参数中传入索引类型比如dict,pd.DataFrame

2. 画图时可以设置的参数，需要记住的[fmt],data,color,marker,linestyle,linewidth,markersize,label,drawstyle

   见官方文档：[https://matplotlib.org/api/_as_gen/matplotlib.pyplot.plot.html?highlight=matplotlib%20pyplot%20plot#matplotlib.pyplot.plot](https://matplotlib.org/api/_as_gen/matplotlib.pyplot.plot.html?highlight=matplotlib pyplot plot#matplotlib.pyplot.plot)

3. 需要记住的画图函数，plot,scatter,fill_between
...
1. pyplot接口的函数都是对当前或最近创建的子图AxesSubplot起作用的
2. pyplot接口进行图形设置时，plt.something()，不输入参数返回当前设置，输入参数进行设置更改
3. 面向对象接口进行图形设置时既可以axe.set_something()，也可以axe.set(something1=,something2=,...) 获取设置用axe.get_something()
4. 需要记住的设置有xlim,xlabel,xticks,xticklabels([],rotation,fontsize),title,
