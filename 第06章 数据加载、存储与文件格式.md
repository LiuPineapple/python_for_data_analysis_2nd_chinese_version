...
python自身与文本和二进制问价的IO见python教程及官方文档：
...
1. 需要记住的参数：path,sep,header,index_col,names,skiprows,na_values,nrows,chunksize,encoding
2. header默认为0，即选取第一行为标题，如果输入header = None且指定了names，那么就用names作为标题，如果没有指定names，pd就自行加上0到N-1的索引
3. index_col用于指定作为索引的列名，如果不输入的话就由pd推断，推断的结果可能是某一列，也有可能是0到N-1
...
1. 需要记住的参数：path,index,header,columns,sep,na_rep
2. index和header的取值是bool，默认是True，把索引和标题传入到文本文件中，可以通过columns指定传入哪几列和列的顺序。
...
几乎对于每种格式的文件（csv,txt,json,pickle等），python自身（依赖于某个库）、pd都有各自的数据IO方式，np对于一些格式的数据也可以IO，的一般看哪种读取方式方便就选择哪种。
