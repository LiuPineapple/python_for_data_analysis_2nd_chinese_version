...
## 元组
元组是一个固定长度，不可改变的Python序列对象。创建元组的最简单方式，是用逗号分隔一列值（加不加圆括号都可以）：
...
1. 几乎所有的序列都可以通过[]的方式索引或切片

2. 只有可变对象可以通过[]的方式更改，不可变对象只能删除，替换不能改变。

3. 存在一种情况，不可变的数据结构中的元素是可变的，这时候可以对其中的某个可变元素进行修改，也是就上面的例子

4. python中内置的数据类型和数据结构本身既是类名，也是类型转换函数，类型转换函数总是创建副本

5. 如果想要创建一个元素的元组，见如下例子
   ...
   即创建一个元素的元组必须在一个元素后面加逗号，否则会被认为是一个数，而创建一个元素的列表不用加逗号
...
1. 需要掌握所有的的list方法：[https://docs.python.org/3.7/tutorial/datastructures.html?highlight=list%20clear](https://docs.python.org/3.7/tutorial/datastructures.html?highlight=list clear)
...
``start``或``stop``都可以被省略，省略之后，分别默认序列的开头和结尾（而且结尾也会被包含）：
...
1. 在序列中，每个值都有其正数位置索引和负数位置索引
2. 序列索引超出范围会报错，但切片和range()函数几乎永远不会报错，只是返回空序列（列表或元组）或空迭代器而已
3. 第三个位置默认为1，为正代表从start只开始正序切片， 为负代表从start值开始倒序切片。如果切片第三个参数是正，前两个参数没给的话，会默认前两个参数是从头到尾，如果第三个参数是负，前两个参数没给的话，会默认前两个参数是从尾到头
...
序列切片和range()函数的三个参数非常相似，不同之处总结如下：

1. 前者是序列切片，后者是创建迭代器，前者参数间用:间隔，后者用,间隔
2. 前者三个参数都可以分别省略，后者第二个参数无法省略
...
In [8]: a = list(range(12))

In [9]: a = list(range(11))
...
1. 除了用于赋值之外，索引和切片产生的都是副本。即如果把索引和切片传递给变量，那么变量的修改将不会影响到原序列
2. data[:]产生复制，即复制了整个序列
...
字典可以通过索引的方法传入新的值，但列表和元组不行，前者会报索引错误，后者本身就不可变
