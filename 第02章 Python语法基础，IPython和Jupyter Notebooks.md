...
3. 显示帮助信息的有?,??,help(),类别信息的有type(),isinstance(),dir()
...
4. 以下例子需要注意 ，int是不可变对象，所以必然不会改变。具体来说，b+1这种算术操作其实是产生了一个新的int对象，b这个变量名指向了新的int对象，原本指向的（也是a指向的）int对象并没有更改
...
1. 函数中只写`return`、 `return None`是作用相同，都是结束执行并返回None
2. 如果不写`return`,python在把整个函数执行完后也会返回None，也就是说python函数一定有返回值
...
2. range(n)里面有n个元素，三个参数中第二个参数不可省略，第一个参数默认是0，三个参数默认是1

3. 如果起点参数与终点参数相等，比如range(10,10),range(0)，不会报错，list转换后是一个空列表，做for循环的话，也不会执行任何操作，for循环中的序号参数（比如i）也不会被赋值。如果起点参数大于终点参数，且步长为正的话，不会报错，list转换后是一个空列表，做for循环的话，也不会执行任何操作，for循环中的序号参数（比如i）也不会被赋值。即range()和序列切片一样，只会产生空但几乎不会报错。

4. for循环的本质是把可迭代对象中的元素依次赋值给前面的序号值（比如i），然后再继续执行循环体内的操作，如果可迭代对象内部为空，那么for循环就不会被执行，前面的序号值也不会被赋值
