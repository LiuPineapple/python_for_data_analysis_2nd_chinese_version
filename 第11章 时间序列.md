...
1. datetime.datetime.now()/toady() 创建一个当前时间的datetime.datetime对象，datetime.date.today()创建一个当前时间的datetime.date对象。

2. 如何创建datetime,date,time,timedelta对象?

   详情见官方文档链接https://docs.python.org/zh-cn/3.7/library/datetime.html
   ...
   #常用的属性和函数
   类属性date.min/max
   实例属性date./year/month/day
   类属性datetime.min/max
   实例属性datetime./year/month/day/hour/minute/second/microsecend/tzinfo
   date/datetime.today()  datetime.now() datetime.utcnow()
   ~~~
...
创建timedelta实例：datetime.timedelta(*days=0*, *seconds=0*, *microseconds=0*, *milliseconds=0*, *minutes=0*, *hours=0*, *weeks=0*)

需要记住的是timedelta.min/max/days/seconds/microseconds属性
...
两种方法把datetime对象转换为固定格式的字符串对象。

两种方法把一定格式的字符串转换为datetime对象。

pd.to_datetime（list of str)把一定格式的字符串列表转换为pd.DatetimeIndex对象
...
date方面，四位数为大写，两位数为小写，time方面，全为大写。
...
pandas自带时间字符串解析功能，所以一般凡是传入时间进行索引/切片的地方，都可以同时传入datetime.datetime对象，Timestamp对象和常用格式的时间字符串对象。
...
---
##### Notes
1. pd里的时间序列频率字符串都可以用大写表示。
2. pd的日期偏移量类似与datetime里的timedelta对象。
...
---
##### Notes
1. Series和DataFrame都有shift方法，对values平移，输入正数是后移操作，负数是前移操作，会出现NaN。也可以输入freq参数实现对索引的平移，防止出现缺失值。
...
---
##### Notes
1. pd.period_range()类似pd.date_range()，前者生成PeriodIndex()，里面的元素是Period，后者生成DatetimeIndex()，里面的元素是Timestamp。
2. pd.PeriodIndex()与pd.DatetimeIndex()类似，生成相应类型的索引，不过前者还要输入频率。
...
---
##### Notes
1. resample方法需要记住的参数：freq,closed,label,kind,loffset
2.  书中有错误，label参数应该是默认left，见官方文档：
